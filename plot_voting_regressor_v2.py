"""
======================================================
Plot individual and voting regression predictions v 02
======================================================
...
def main(args):
    ...
    predictMode = args.predict_mode
    test_split = args.test_split

    print('Input xlsx file:', input_xlsx_file)
    print('Model file:', model_file)
    print('Bin size:', str(binSize))
    print('Random state:', str(randState))
    print('Max iterations for MLP:', str(maxIterations))
    print('Prediction mode:', str(predictMode))
    ...
    if predictMode:
        if model_file is None:
            print('\nYou must enter the prefix of a saved model file before you can make predictions.\n')
            sys.exit(2)
        if plot_type == 'fold':
            print('\nPlot type cannot be fold when making predictions.\n')
            sys.exit(2)
    ...
    if testMode: print('Number of bins:', numberOfBins)
    ...
    data_file_name = input_xlsx_file
    if predictMode:
        sheetList = ['1086T']
    else:
    ...
    for sheet in sheetList:
        ...
        for index, row in df.iterrows():
            ...
            # Don't set y array if in predictMode
            if not predictMode:
                ...
                if testMode: print(sample, nMol_measured, nMol_actual, nMol_after_sequencing)
    ...
    # Plot normalized size distribution
    # plt.figure()
    # plt.yscale('log')
    # plt.plot(X_normalized_array)
    # plt.title('Size distribution for all samples')
    ...
    # If in predict mode, predict on all samples
    if predictMode:
        X_test = X
        print('X test shape:', X_test.shape)
    else:
        # Split data into training and test sets
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_split, random_state = randState)
        # print('tDF:', yDF.head)
        # print('Y:', y)
        # print(y_test)
        ...
        if model_file is not None:
            with open(model_file + '_mlp.pkl', 'wb') as file: pickle.dump(reg_mlp, file)
            with open(model_file + '_gb.pkl', 'wb') as file: pickle.dump(reg_gb, file)
            with open(model_file + '_rf.pkl', 'wb') as file: pickle.dump(reg_rf, file)
            with open(model_file + '_ab.pkl', 'wb') as file: pickle.dump(reg_ab, file)
            with open(model_file + '_kn.pkl', 'wb') as file: pickle.dump(reg_kn, file)
            with open(model_file + '_lr.pkl', 'wb') as file: pickle.dump(reg_lr, file)
            with open(model_file + '_ereg.pkl', 'wb') as file: pickle.dump(ereg, file)

    # %%
    # Making predictions
    # --------------------------------
    #
    # Now we will use each of the regressors to predict on the test set
    # xt = X[:21]
    if predictMode:
        # Restore models
        with open(model_file + '_mlp.pkl', 'rb') as file: reg_mlp = pickle.load(file)
        with open(model_file + '_gb.pkl', 'rb') as file: reg_gb = pickle.load(file)
        with open(model_file + '_rf.pkl', 'rb') as file: reg_rf = pickle.load(file)
        with open(model_file + '_ab.pkl', 'rb') as file: reg_ab = pickle.load(file)
        with open(model_file + '_kn.pkl', 'rb') as file: reg_kn = pickle.load(file)
        with open(model_file + '_lr.pkl', 'rb') as file: reg_lr = pickle.load(file)
        with open(model_file + '_ereg.pkl', 'rb') as file: ereg = pickle.load(file)
    ...
    if plot_type == 'fold':
        ...
        if not predictMode: plt.plot(y_test_array / y_test_array, 'r*', ms=10, label='Actual')
        ...
        if not predictMode: plt.semilogy(y_test_array, 'r*', ms=10, label='Actual')
        ...
        if not predictMode: plt.plot(y_test_array, 'r*', ms=10, label='Actual')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-x", "--input_xlsx_file", help="Spreadsheet containing data", default="ATACseq_molarity_estimation.xlsx")
    parser.add_argument("-m", "--model_file", help="Pickle file for saved models")
    parser.add_argument("-r", "--random_state", help="Random state initializer", type=int, default=1)
    parser.add_argument("-b", "--bin_size", help="Bin size for RFU distribution", type=int, default=50)
    parser.add_argument("-i", "--max_iterations", help="Maximum number of iterations for MLP regressor", type=int, default=10000)
    parser.add_argument("-p", "--predict_mode", help="Turn prediction mode on", action="store_true")
    parser.add_argument("-y", "--plot_type", help="Type of plot (regular / log / fold change", choices=["regular", "log", "fold"], default="regular")
    parser.add_argument("-s", "--test_split", help="Proportion of samples used in test set", type=float, default=0.1)
